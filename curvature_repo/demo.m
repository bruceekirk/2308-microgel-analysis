...
%--------------------------------------------------------------------------
% Image files
I = imread('squish.png');    %'2019_04_19_19_33_26_456_Ang_0.bmp');
...
Z = zeros(size(X));
C_raw = shape_details.curvature'*1;
C = smooth(C_raw, 50, 'lowess');  % Applying smoothing to the curvature data
...
surf([X(:) X(:)], [Y(:) Y(:)], [Z(:) Z(:)], [C C], ...  % Reshape and replicate data
 ...
 'LineWidth', 12);            % Make a thicker line
hold off
